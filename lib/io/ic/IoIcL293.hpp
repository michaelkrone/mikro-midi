...
static const uint8_t IcL293MotorNb = 2;

class IcL293 : public Ic {
  protected:
  Output* mEnable[IcL293MotorNb];
  Output* mDriverInput[IcL293MotorNb][2];
  bool mEnabled[2];

  public:
  IcL293(
      Output* enableA, Output* driverInputA1, Output* driverInputA2,
  ...
  inline void stopAll() {
    for (uint8_t i = 0; i < IcL293MotorNb; i++) {
      enable(i);
      stop(i);
      disable(i);
    }
